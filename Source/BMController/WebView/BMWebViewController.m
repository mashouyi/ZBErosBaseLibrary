...
@property (nonatomic, copy) BMAppDelegate *delegate;
...
- (instancetype)initWithRouterModel:(BMWebViewRouterModel *)model
{
    if (self = [super init])
    {
        ...
        [self subInit];
        _delegate = (BMAppDelegate *)[UIApplication sharedApplication].delegate;
        _delegate.isLand = self.routerInfo.isLand;
        [self setNewOrientation: self.routerInfo.isLand];
    }
    return self;
}

...
@property (nonatomic, strong) NSArray *tipArrs;
@property (nonatomic, strong) NSString *downLoadUrl;
@property (nonatomic, strong) VersionCheckView *checkView;
            ...
            if (configData.length > 0) {
                ...
                if (writeSuccess) {
                    ...
                    if (!TK_PlatformInfo().customBundleUpdate) {
                       [[BMMediatorManager shareInstance] showJsResourceUpdatedAlert:_tipArrs];
                    }
                }
            }
        }
...
//        }
        ...
        if([self isNullToDictionary:data]){
            return;
        }
        
        _downLoadUrl = data[@"path"];
        _tipArrs = [data[@"msg"] componentsSeparatedByString:@"#"];
        if([data[@"upgrade"] isEqualToString:@"patch"]){
            ...
            self.checkView.dataArrs = _tipArrs;
            [window addSubview:self.checkView];
        }
...
#pragma mark----立即更新打开本地浏览器

- (void)updateBtnIsClick{
    [self.checkView removeFromSuperview];
    if(![_downLoadUrl isEqualToString:@""]){
        if (@available(iOS 10.0, *)) {
            [[UIApplication sharedApplication]openURL:[[NSURL alloc] initWithString:_downLoadUrl] options:@{} completionHandler:^(BOOL success) {
...
- (VersionCheckView *)checkView{
    if(!_checkView) {
        _checkView = [[VersionCheckView alloc]initWithFrame:[UIScreen mainScreen].bounds];
        _checkView.delegate = self;
        _checkView.btnTitle = @"立即体验";
    }
    return  _checkView;
}
- (NSArray *)tipArrs{
    if (!_tipArrs) {
        _tipArrs = [[NSArray alloc]init];
    }
    return _tipArrs;
}
@end
